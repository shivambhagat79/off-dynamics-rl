...
import algo.cfn_utils as cfn_utils
import setproctitle
...
if __name__ == "__main__":
    ...
    cfn_buffer = cfn_utils.CFNReplayBuffer(capacity=int(1e6), device=device)

    # Initialise CFN
    cfn = cfn_utils.CoinFlipNetwork(state_dim, config['coin_flips_dim']).to(device)
    cfn_optimizer = torch.optim.Adam(cfn.parameters(), lr=config['cfn_lr'])
    ...
    for t in range(int(config['max_step'])):
        ...
        if t % config['tar_env_interact_interval'] == 0:
            ...
            tar_next_state_tensor = torch.FloatTensor(np.array(tar_next_state)).to(device).unsqueeze(0)
            intrinsic_reward = cfn.intrinsic_bonus(tar_next_state_tensor)
            tar_reward += config['lambda_cfn'] * intrinsic_reward.item()

            # Calculate Priority
            priority = config['alpha_cfn'] + (1- config['alpha_cfn']) * (intrinsic_reward.item() ** 2)
            coin_flips = torch.randint(0, 2, (config['coin_flips_dim'],)) * 2 - 1
            cfn_buffer.add(priority, tar_state, coin_flips)
        ...
        if (len(cfn_buffer) >= config['cfn_batch_size']):
            cfn_batch, n_samples, tree_indexes = cfn_buffer.sample(config['cfn_batch_size'])
            # Forward
            state_batch, c_batch = cfn_batch
            preds = cfn(state_batch)
            loss = F.mse_loss(preds, c_batch)

            # Backprop through f_hat only
            cfn_optimizer.zero_grad()
            loss.backward()
            cfn_optimizer.step()

            # Update priorities
            rewards = cfn(state_batch).detach()
            new_priorities = config['alpha_cfn']*(n_samples+1) + (1 - config['alpha_cfn']) * (rewards.pow(2).sum(dim=1) / config['coin_flips_dim']).cpu().numpy()
            cfn_buffer.update_priorities(tree_indexes, new_priorities)
