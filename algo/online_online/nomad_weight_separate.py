...
class NOMAD_WEIGHT_SEPARATE(object):
    """
    NOMAD with SEPARATE critics for exploration and task policies.
    
    Key features:
    - Separate critics: no gradient interference between exploration and task
    - Within-domain metric learning: learns target state distances via permutation
    - Fixed target initial state reference: consistent intrinsic reward signal
    """
    ...
    def _update_liberty_models(self, state, action, reward, next_state, writer):
        ...
        # Update metric model - WITHIN-DOMAIN learning via permutation
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, initial_state, batch_size=128, writer=None):
        ...
        with torch.no_grad():
            # Add intrinsic reward using fixed target initial state reference
        ...
        with torch.no_grad():
            ...
            if writer is not None and self.total_it % 1000 == 0:
                writer.add_scalar('nomad_weight_sep/is_weights_src_mean', is_weights_src.mean().item(), self.total_it)
                writer.add_scalar('nomad_weight_sep/is_weights_tar_mean', is_weights_tar.mean().item(), self.total_it)
                # writer.add_scalar('nomad_weight_sep/action_noise_mean', noise.abs().mean().item(), self.total_it)
                writer.add_scalar('nomad_weight_sep/temperature', self.temp, self.total_it)
                writer.add_scalar('nomad_weight_sep/exploration_q_loss', q_loss_exploration.item(), self.total_it)
                if initial_state is not None:
                    writer.add_scalar('nomad_weight_sep/intrinsic_reward_mean', intrinsic_reward.mean().item(), self.total_it)
                    writer.add_scalar('nomad_weight_sep/potential_mean', potential_s.mean().item(), self.total_it)
        ...
        if self.total_it % self.config.get('policy_freq', 2) == 0:
            ...
            if writer is not None:
                writer.add_scalar('nomad_weight_sep/task_q_loss', total_q_loss_task.item(), self.total_it)
                writer.add_scalar('nomad_weight_sep/task_policy_loss', total_policy_loss.item(), self.total_it)
                writer.add_scalar('nomad_weight_sep/exploration_policy_loss', policy_loss_exploration.item(), self.total_it)
