...
class CFN(object):
    ...
    def __init__(self,
        ...
        self.exploration_policy_optimizer = torch.optim.Adam(self.exploration_policy.parameters(), lr=config['exploration_actor_lr'])
    ...
    def update_policy_and_temp(self, state_batch, policy, writer=None):
        action_batch, logprobs_batch, _ = policy(state_batch, get_logprob=True)
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, batch_size=128, writer=None):
        ...
        # --- Train exploration policy on target data ---
        q_loss_exp = self.update_q_functions(tar_state, tar_action, tar_reward, tar_next_state, tar_not_done, writer)
        ...
        exp_pi_loss, exp_a_loss = self.update_policy_and_temp(tar_state, self.exploration_policy)

        self.exploration_policy_optimizer.zero_grad()
        exp_pi_loss.backward()
        ...
        # --- Train main policy with importance sampling ---
        src_state, src_action, src_next_state, src_reward, src_not_done = src_replay_buffer.sample(batch_size)

        # Combine source and target batches for main policy training
        ...
        q_loss_main = self.update_q_functions(state, action, reward, next_state, not_done, writer)

        # Calculate importance weights for target samples
        with torch.no_grad():
            # Log-probabilities of target actions under the main policy
            _, main_logprob, _ = self.policy(tar_state, get_logprob=True)
            # Log-probabilities of target actions under the exploration policy
            _, exp_logprob, _ = self.exploration_policy(tar_state, get_logprob=True)

            importance_weights = torch.exp(main_logprob - exp_logprob).clamp(0.1, 10.0)

            # Create a combined weight tensor
            src_weights = torch.ones(batch_size, 1, device=self.device)
            combined_weights = torch.cat([src_weights, importance_weights], 0)

        # Apply importance weights to the Q-loss for the main policy
        q_loss_main_weighted = (q_loss_main * combined_weights).mean()

        self.q_optimizer.zero_grad()
        q_loss_main_weighted.backward()
        self.q_optimizer.step()

        self.update_target()

        # Freeze Q-networks for main policy update
        for p in self.q_funcs.parameters():
            p.requires_grad = False

        main_pi_loss, main_a_loss = self.update_policy_and_temp(state, self.policy)

        # Apply importance weights to the policy loss
        main_pi_loss_weighted = (main_pi_loss * combined_weights).mean()

        self.policy_optimizer.zero_grad()
        main_pi_loss_weighted.backward()
        self.policy_optimizer.step()

        if self.config['temperature_opt']:
            self.temp_optimizer.zero_grad()
            main_a_loss.backward()
            self.temp_optimizer.step()
