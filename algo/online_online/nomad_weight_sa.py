...
class NOMAD_WEIGHT_SA(object):
    ...
    def update_classifier(self, src_replay_buffer, tar_replay_buffer, batch_size, writer=None):
        ...
        sa_logits = self.classifier(state_batch, action_batch, nextstate_batch, with_noise=True)
        loss = F.cross_entropy(sa_logits, label_batch)
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, initial_state, batch_size=128, writer=None):
        ...
        with torch.no_grad():
            ...
            sa_probs_src = self.classifier(src_state, src_action, src_next_state, with_noise=False)
            darc_log_ratio_src = torch.log(sa_probs_src[:, :1] + 1e-8) - torch.log(sa_probs_src[:, 1:] + 1e-8)
