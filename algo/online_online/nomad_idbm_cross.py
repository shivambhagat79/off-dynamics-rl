...
class NOMAD_IDBM_CROSS(object):
    """
    NOMAD-IDBM with SEPARATE critics AND CROSS-DOMAIN intrinsic rewards.
    
    Key improvements:
    1. Separate critics for exploration and task policies (no gradient interference)
    2. Cross-domain potential function using source states as reference
    3. Intrinsic reward encourages target states far from source domain
    4. Metric trained on cross-domain pairs for proper distance measurement
    """
    ...
    def _update_liberty_models(self, src_data, tar_data, writer):
        ...
        # Update metric model - CROSS-DOMAIN bisimulation metric
        # Compare Source states to Target states
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, initial_state, batch_size=128, writer=None):
        ...
        # Uses EXPLORATION CRITIC with CROSS-DOMAIN intrinsic rewards
        tar_state, tar_action, tar_next_state, tar_reward, tar_not_done = tar_replay_buffer.sample(batch_size)

        with torch.no_grad():
            # CROSS-DOMAIN intrinsic reward: Sample source states as reference
            src_reference, _, _, _, _ = src_replay_buffer.sample(batch_size)
            
            # Compute cross-domain potential: distance from source to target
            potential_s = self.metric_model(src_reference, tar_state)
            potential_s_next = self.metric_model(src_reference, tar_next_state)
            
            # Intrinsic reward: encourages moving to target states far from source
        ...
        # Uses TASK CRITIC with EXTRINSIC rewards only and importance sampling
        ...
        with torch.no_grad():
            # Q-target for source data using TASK critic (EXTRINSIC reward only)
            ...
            # Q-target for target data using TASK critic (EXTRINSIC reward only - NO intrinsic bonus)
            ...
            if writer is not None and self.total_it % 1000 == 0:
                writer.add_scalar('nomad_cross/is_weights_src_mean', is_weights_src.mean().item(), self.total_it)
                writer.add_scalar('nomad_cross/is_weights_tar_mean', is_weights_tar.mean().item(), self.total_it)
                writer.add_scalar('nomad_cross/action_noise_mean', noise.abs().mean().item(), self.total_it)
                writer.add_scalar('nomad_cross/temperature', self.temp, self.total_it)
                writer.add_scalar('nomad_cross/exploration_q_loss', q_loss_exploration.item(), self.total_it)
                writer.add_scalar('nomad_cross/intrinsic_reward_mean', intrinsic_reward.mean().item(), self.total_it)
                writer.add_scalar('nomad_cross/potential_mean', potential_s.mean().item(), self.total_it)
        ...
        if self.total_it % self.config.get('policy_freq', 2) == 0:
            ...
            if writer is not None:
                writer.add_scalar('nomad_cross/task_q_loss', total_q_loss_task.item(), self.total_it)
                writer.add_scalar('nomad_cross/task_policy_loss', total_policy_loss.item(), self.total_it)
                writer.add_scalar('nomad_cross/exploration_policy_loss', policy_loss_exploration.item(), self.total_it)
