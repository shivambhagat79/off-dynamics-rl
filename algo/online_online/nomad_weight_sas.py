...
class NOMAD_WEIGHT_SAS(object):
    ...
    def update_classifier(self, src_replay_buffer, tar_replay_buffer, batch_size, writer=None):
        ...
        sas_logits = self.classifier(state_batch, action_batch, nextstate_batch, with_noise=True)
        loss = F.cross_entropy(sas_logits, label_batch)
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, initial_state, batch_size=128, writer=None):
        ...
        with torch.no_grad():
            ...
            sas_probs_src = self.classifier(src_state, src_action, src_next_state, with_noise=False)
